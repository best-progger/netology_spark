...
df2 = df1.groupBy(df1.location.alias('country')).agg(F.max('new_cases').alias('m_new_cases'))
#Джоин датафреймов и отбор нужных полей, вывод топ 10
df1.join(df2,(df1.location == df2.country) & (df1.new_cases == df2.m_new_cases),'inner').select(df1.date,df1.location,df2.m_new_cases).sort(F.col('m_new_cases').desc()).show(10)
